...
class Predictor:
    ...
    def update(self, start, end, img_curr, img_goal):
        '''
        update the weight of LSTM and return the estimated COM
        '''
        bs = self.bs
        A1 = []
        I1 = []
...
class PushController:
    def __init__(self):
        ...
        self.img_com_pub = rospy.Publisher('img_com', Image, queue_size=10)
        ...
        self.com_np = [0.5, 0.5] ## initialize to be center of an image
        ...
        self.push_srv = rospy.Service('fetch_push', FetchPush, self.push_callback)
    ...
    def get_next_mask(self, points, xc, yc):
        return self.sensor.generate_next_image(points, xc, yc)
    ...
    def draw_com(self, img, com_np, moved_x, moved_y):
        '''draw predicted COM on the next perceived image'''
        (yy, xx) = np.where(img>0)
        img_3d = np.zeros((int(H), int(W), 3))
        img_3d[yy, xx] = np.array([255,255,255])

        com_x = int(com_np[0] * W) - int(- moved_y  / self.sensor.meters_per_pixel)
        com_y = int(com_np[1] * H) - int(- moved_x  / self.sensor.meters_per_pixel)

        cv2.circle(img_3d, (com_x, com_y), 3, (50, 205, 50), -1)
        img_3d = img_3d.astype(np.uint8)
        #cv2.imwrite(self.img_path + '_com_' + str(self.curr_step) + '.jpg', img_3d)

        ## publish image
        ros_img = self.bridge.cv2_to_imgmsg(img_3d, encoding='bgr8')
        for i in range(10):
            self.img_com_pub.publish(ros_img)

        '''
        adjustable threshold to decide whether rotating an object is necessary
        '''
        print 'com is offset by ', com_x - int(W/2)
        print int(W/25)
        return (int(W/2) - com_x) > int(W/25) ## com is on the left side of object, dangerous to push

    def prob_com(self, xc_prev, yc_prev, UP=True, side_prob=False):
        '''
        apply probing actions to estimate COM
        '''
        global MODE
        bs = args.batch_size
        ...
        ## fit mask to a rotated rectangle
        angle, w, l, theta = get_rot_rect(curr_mask.copy())
        print '+++++++'
        print angle, w, l
        side_prob = (abs(angle - 90) < 1 or abs(angle + 90) < 1 or side_prob)
        print 'side prob is ', side_prob
        ...
        if not side_prob:
            ## push perpendicular through the center via middle points on the longer side
            if UP: ## push in positive x direction wrt robot
                if math.cos(angle) < 0:
                    start_x = W/2.0 - w * math.sin(angle)
                    start_y = H/2.0 - w * math.cos(angle)
                    end_x = W/2.0 + w/2 * math.sin(angle)
                    end_y = H/2.0 + w/2 * math.cos(angle)
                else:
                    start_x = W/2.0 + w * math.sin(angle)
                    start_y = H/2.0 + w * math.cos(angle)
                    end_x = W/2.0 - w/2 * math.sin(angle)
            ...
            if UP: # push in positive y direction wrt robot
                start_x = W/2.0 - w
                start_y = H/2.0
        ...
        _, img_in_curr = cv2.threshold(curr_mask.copy(), 30, 255, cv2.THRESH_BINARY)

        ## draw COM
        self.draw_com(img_in_curr, self.com_np, xc-xc_prev, yc-yc_prev)

        ## execute probing action
        action_value_pairs = [[[start_pt, end_pt], 1]]
        best_start, best_end = self.execute_push(action_value_pairs, xc, yc, img_in_curr.copy(), tune_enable=False)

        ## update the network and COM
        points = self.get_points()
        next_mask = self.get_next_mask(points, xc, yc)
        next_mask = next_mask.astype(np.uint8)
        angle_after, _, _, theta_after = get_rot_rect(next_mask.copy())

        CW = None
        print theta, theta_after
        if abs(theta_after - theta) < 90:
            if abs(theta_after - theta) < 10:
                CW = True
            else:
                if theta_after < theta:
                    CW = False
                else:
                    CW = True
        else:
        ...
        #_, img_in_next = cv2.threshold(next_mask.copy(), 30, 255, cv2.THRESH_BINARY)
        self.com_np = self.pred.update(best_start, best_end, img_in_curr, img_in_curr)

        return xc, yc, CW, side_prob

    def rotate_inplace(self, curr_x, curr_y, rot_angle):
        ...
        while True and not rospy.is_shutdown():
           ...
           diff_tran, error_w = get_img_transform(img_in_curr, goal_mask)
           ...
           if abs(error_w) < 25:
               print 'COMMAND: EXIT EXECUTION'
               break

           moved_x = xc - moved_x
           moved_y = yc - moved_y
           rotate = self.draw_com(img_in_curr, self.com_np, moved_x, moved_y)
           #raw_input('wait')
           ...
           if abs(error_w) < 20: ## degree
               self.dw = error_w
           else:
               self.dw = error_w / abs(error_w) * 40
           ...
           #else:
               #MODE = 'wxy'
           print 'dx, dy, dw: ', self.dx, self.dy, self.dw
           ...
           if best_start == None or best_end == None:
               ...
               self.pred.model.hidden = hidden
               #points = self.get_points()
               #next_mask = get_next_mask(points, xc, yc)
               #next_mask = next_mask.astype(np.uint8)
               #_, img_in_next = cv2.threshold(next_mask.copy(), 30, 255, cv2.THRESH_BINARY)
               self.com_np = self.pred.update(best_start, best_end, img_in_curr, img_in_next)
               moved_x = xc
               moved_y = yc
    ...
    def run(self, req=None):
        ...
        # Step 3: generate goal mask
        goal_mask = self.get_target_mask(curr_mask, gw, int(-gy / self.sensor.meters_per_pixel), int(-gx / self.sensor.meters_per_pixel))
        ''' rotate a bit to estimate COM'''

        rotate = None
        #ggw = get_goal_w(curr_mask.copy())
        #angle = ggw / abs(ggw) * 40.0
        #print '##COMMAND: ROTATE A BIT BY ', angle
        if direction == 0:
            print '##COMMAND: ESTIMATE COM'
            moved_x, moved_y, R1, side_prob_ = self.prob_com(xc, yc, UP=True) #moved_x, moved_y are previous xc, yc values
            moved_x, moved_y, R2, _ = self.prob_com(moved_x, moved_y, UP=False, side_prob=side_prob_) #moved_x, moved_y are previous xc, yc values

            print R1, R2
            #if R1 and (not R2) or (R1 and R2):
            if (not R1) and R2:
                rotate = True
            else:
                rotate = False

        #moved_x, moved_y = self.rotate_inplace(xc, yc, angle, absolute=False)
        ...
        while True and not rospy.is_shutdown():
           ...
           moved_y = yc - moved_y
           #rotate = self.draw_com(img_in_curr, self.com_np, moved_x, moved_y)
           self.draw_com(img_in_curr, self.com_np, moved_x, moved_y)
           #raw_input('wait')
           ...
           if direction == 0:
               if rotate and rotation_needed: ## need to rotate object
                   #MODE = 'wxy'
                   print '##COMMAND: IN-PLACE ROTATION'
                   #error_w = 180
                   ''' repeat two times to avoid angle ambuiguation from symmetric objects
                   '''
                   ggw = get_goal_w(curr_mask.copy())
                   angle = 0
                   if ggw < 0:
                       ...
                       angle = (-180.0 + ggw) / 3.0
                   print 'Rotate 2 time and each by ', angle
                   moved_x, moved_y = self.rotate_inplace(xc, yc, angle)
                   moved_x, moved_y = self.rotate_inplace(moved_x, moved_y, angle)
                   moved_x, moved_y = self.rotate_inplace(moved_x, moved_y, angle)
               ...
               error_x = xm - xc ## don't care about x position
               error_y = ym - yc
               error_w = 0
           ...
           #### execute action
           best_start, best_end = self.execute_push(action_value_pairs, xc, yc, img_in_curr.copy())
           if best_start == None or best_end == None:
               print 'sample action again'
           else:
               self.pred.model.hidden = hidden
    ...
    def grasp_from_side(self, side):
        ...
        while not rospy.is_shutdown():
            try:
                time = self.sensor.tl.getLatestCommonTime(self.robot_frame, self.sensor.world_frame)
                center.header.stamp = time
                center_transform = self.sensor.tl.transformPoint(self.robot_frame, center)
                break
    ...
    def execute_push(self, sorted_actions, xc, yc, img, tune_enable=True):
           pack = sorted_actions.pop(0)
           best_start = pack[0][0]
           best_end = pack[0][1]
           self.draw_action(img.copy(), best_start, best_end, last=True)

           #raw_input('wait before execution')
           #rospy.sleep(1)
           ...
           if self.left:
               self.arm.move_to_joint_goal(HIDE_LEFT_JOINTS, replan=True, execution_timeout=20)
           else:
               self.arm.move_to_joint_goal(HIDE_RIGHT_JOINTS, replan=True, execution_timeout=20)
